...
<template>
  <view class="wrapper">
    ...
    <view
      :style="[Tactive === index ? rightStyle : '']"
      class="delete"
      @click.stop="operation(itemData)"
    >
      <image
...
export default {
  props: {
    ...
    operationImageSrc: {
      type: String,
      default: 'https://education-1323054692.cos.ap-beijing.myqcloud.com/mini/static/del_icon.png'
    },
  ...
  methods: {
    ...
    touchMove(e, index) {
      ...
      if (Math.abs(this.startM - this.startX) < 30) {
        return
      }
