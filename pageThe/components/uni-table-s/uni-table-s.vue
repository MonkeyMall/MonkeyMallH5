    ...
    <view class="s-table" v-if="!fixedAttrNum">
      <view class="s-table-row display-flex">
        <template v-if="className === 'header-fixed-1'">
          <block v-for="(item, index) in headerData" :key="index">
            <view v-if="!item.childrens" class="s-table-row-header s-table-row-item flex1 center">
              {{ item.name }}
            </view>
            <view v-else class="s-table-row-header flex2 center">
              <view class="s-table-header-lists center">{{ item.name }}33</view>
              <view class="s-table-fixed-header-children">
                <view v-for="(item1, index1) in item.childrens" :key="index1"
        ...
        <template v-else>
          <view
            :class="isSetWidth && (item == '年份' || item === '研究方向') ? 's-table-row-header s-table-row-item noflex1 center' : 's-table-row-header s-table-row-item flex1 center'"
      ...
      <view :class="list.length ? 'list-box' : 'empty list-box'">
        <template v-if="list.length">
          <view class="s-table-row display-flex" v-for="(item, index) in list" :key="index">
            <view
              :class="isSetWidth && (item1 == 'adjustYear' || item1 === 'researchDirection') ? 's-table-row-con s-table-row-item noflex1 center' : 's-table-row-con s-table-row-item flex1 center'"
              v-for="(item1, index1) in headerAttr" :key="index1">
              <template v-if="item1 === 'image'">
                <image :src="item[item1]" mode="scaleToFill" />
              </template>
              <view v-else-if="Array.isArray(item[item1])">
                <view v-for="(item2, index2) in item[item1]" :key="index2"
              ...
              <template v-else>
                <view v-if="item[item1] && item[item1] != null">
                  <view v-if="!isUvParse">{{ item[item1] }}</view>
                  <uv-parse v-else :content="item[item1]"></uv-parse>
                </view>
                <view v-else>--</view>
              </template>
            </view>
      ...
      <view class="s-table-row">
        ...
        <view class="scrollBox">
          <view class="box headerBox">
            <!-- v-if="index >= fixedAttrNum" -->
            <view v-for="(item, index) in headerData" :key="index" class="s-table-row-header s-table-row-item center">{{
      item }}</view>
          </view>
          <view :class="!list.length ? 'list-box empty' : 'list-box'">
            <template v-if="list.length">
              <view :class="[index === list.length ? 'last' : '', 'box con', 'con' + index]"
                v-for="(item, index) in list" :key="index">
                <view v-for="(item1, index1) in headerAttr" :key="index1" class="s-table-row-item center">
