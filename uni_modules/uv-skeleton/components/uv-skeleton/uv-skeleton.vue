<template>
  <view class="uv-skeleton">
    <view class="uv-skeleton__wrapper" ref="uv-skeleton__wrapper" v-if="loading"
      style="display: flex; flex-direction: row;">
      <view class="uv-skeleton__wrapper__avatar" v-if="avatar"
        :class="[`uv-skeleton__wrapper__avatar--${avatarShape}`, animate && 'animate']" :style="{
      ...
      <view class="uv-skeleton__wrapper__content" ref="uv-skeleton__wrapper__content" style="flex: 1;">
        <view class="uv-skeleton__wrapper__content__title" v-if="title" :style="{
							width: uTitleWidth,
							height: $uv.addUnit(titleHeight),
						}" :class="[animate && 'animate']"></view>
        <view class="uv-skeleton__wrapper__content__rows" :class="[animate && 'animate']"
    ...
    computed: {
      rowsArray() {
        ...
        for (let i = 0; i < this.rows; i++) {
          let item = {},
            // 需要预防超出数组边界的情况
            rowWidth = this.$uv.test.array(this.rowsWidth) ? (this.rowsWidth[i] || (i === this.row - 1 ? '70%' :
              '100%')) : i ===
